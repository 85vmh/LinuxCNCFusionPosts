...
description = "LinuxCNC - with optional 4th axis";
vendor = "LinuxCNC";
...
properties = {
  ...
  has4thAxis : true, // enable 4th axis support
  goToPredefinedPosition : false, // move the table to predefined position when program complete
  pathBlending: true //the default value for G64
};
...
propertyDefinitions = {
  ...
  pathBlending: {title:"Path Blending: G64 P0.01", description:"When on, will set the G64 P0.01", type:"boolean"},
  has4thAxis: {title:"Has 4th Axis", description:"Enable the usage of a 4th axis along X axis.", type:"boolean"},
  goToPredefinedPosition: {title:"GoTo: Predefined position (G30)", description:"Move the table to predefined position after running a program.", type:"boolean"},
...
function onOpen() {
  ...
  if (properties.has4thAxis) { // note: setup your machine here
  ...
  if (programName) {
    ...
    writeComment("Time Posted: " + datestring);
  }
  if (programComment) {
  ...
  if (properties.writeTools) {
    ...
    if (tools.getNumberOfTools() > 0) {
      for (var i = 0; i < tools.getNumberOfTools(); ++i) {
        ...
        var comment = "T" + toolFormat.format(tool.number) + " - " + getToolTypeName(tool.type) + " -> "
        
        comment += "[DIAM=" + xyzFormat.format(tool.diameter)
        comment += " FLUTES=" + xyzFormat.format(tool.numberOfFlutes)
  ...
  if(properties.pathBlending){
    writeBlock(gFormat.format(64), "P0.01"); // value for Path blending
  }
}
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    writeBlock(mFormat.format(6), "T" + toolFormat.format(tool.number), gFormat.format(43));
    if (tool.comment) {
...
function onClose() {
  ...
  // stop the spindle and then retract
  onCommand(COMMAND_STOP_SPINDLE);
  ...
  if (properties.goToPredefinedPosition){
